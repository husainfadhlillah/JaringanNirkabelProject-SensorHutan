...
void PacketForwarder::initialize(int stage)
{
    if (stage == 0) {
        ...
        LoRa_PacketReceivedPerNode = registerSignal("LoRa_PacketReceivedPerNode");
...
void PacketForwarder::updateNodeStatistics(const MacAddress& nodeAddr, simtime_t rcvTime)
{
    auto& stats = nodeStatistics[nodeAddr];
    stats.packetsReceived++;

    if (stats.firstPacketTime == 0) {
        stats.firstPacketTime = rcvTime;
    }
    stats.lastPacketTime = rcvTime;

    // Emit signal with node address as context
//    emit(LoRa_PacketReceivedPerNode, stats.packetsReceived, nodeAddr.str().c_str());
}
...
void PacketForwarder::processLoraMACPacket(Packet *pk)
{
    ...
    updateNodeStatistics(frame->getTransmitterAddress(), simTime());
...
void PacketForwarder::finish()
{
    ...
    // Record per-node statistics
    for (const auto& nodeStat : nodeStatistics) {
        std::string nodeAddr = nodeStat.first.str();
        const NodeStats& stats = nodeStat.second;

        recordScalar(("Packets_Received_Node_" + nodeAddr).c_str(), stats.packetsReceived);
        recordScalar(("First_Packet_Time_Node_" + nodeAddr).c_str(), stats.firstPacketTime.dbl());
        recordScalar(("Last_Packet_Time_Node_" + nodeAddr).c_str(), stats.lastPacketTime.dbl());
    }
}
